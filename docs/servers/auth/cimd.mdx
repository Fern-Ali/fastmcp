...
Enable CIMD support in your OAuth proxy by simply including it—no additional configuration required for basic usage:
...
<ParamField body="--redirect-uri" type="str" required>
  Authorized redirect URI. Can be used multiple times. Supports wildcards like `http://localhost:*/callback`
</ParamField>
